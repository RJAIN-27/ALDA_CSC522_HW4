...
alda_kmeans_elbow_plot <- function(data_df, k_values){
  ...
  for (val in k_values)
  {
    vector <- c(vector,  alda_calculate_sse(data_df,alda_cluster(data_df = data_df, n_clusters = val, clustering_type = "kmeans")))
  }
  
  png("G41_elbow.png")
  plot(k_values,vector, type="b", pch=19, frame=FALSE, xlab="Number of Clusters", ylab=" Total SSE", main = "Elbow Plot")
  dev.off()
}
...
alda_cluster <- function(data_df, n_clusters, clustering_type){
  ...
  if(clustering_type == "kmeans"){
    ...
    (kmeans(data_df, n_clusters, iter.max = 100, nstart = 1,algorithm = c("Lloyd"), trace=False))$cluster
    ...
    plot(hclust_avg, hang=-1)
    rect.hclust(hclust_avg,k=n_clusters)
    return (cut_avg)
...
alda_svm <- function(x_train, x_test, y_train, kernel_name){
  ...
  gamma_values = c(0.05, 0.5, 1, 2)
  degree_values = c(1,2,3)
  if(kernel_name == "radial"){
    # ~1-2 lines 
    model = best.tune(svm, x_train, y_train, ranges = list(gamma = gamma_values, cost = cost_values, kernel = 'radial'), tune.control(sampling = "cross", cross = 10))
  }else if(kernel_name == 'polynomial'){
    #~1-2 lines
    model = best.tune(svm, x_train, y_train, ranges = list(gamma = gamma_values, cost = cost_values, degree = degree_values, kernel = 'polynomial'),tune.control(sampling = "cross", cross = 10))
  }else if(kernel_name == 'sigmoid'){
    #~1-2 lines
    model = best.tune(svm, x_train, y_train, ranges = list(gamma = gamma_values, cost = cost_values, kernel = 'sigmoid'), tune.control(sampling = "cross", cross = 10))
  }else{ # default linear kernel
    #~1-2 lines
    model = best.tune(svm, x_train, y_train, ranges = list(cost = cost_values, kernel = 'linear'), tune.control(sampling = "cross", cross = 10))
  }
  return(list(model, factor(predict(model, x_test))))
...
classification_compare_accuracy <- function(y_test, linear_kernel_prediction, radial_kernel_prediction, 
  ...
  linear_pred = y_test == linear_kernel_prediction
  linear_accuracy = length(linear_pred[linear_pred==TRUE])/length(linear_pred)*100
  
  polynomial_pred = y_test == polynomial_kernel_prediction
  polynomial_accuracy = length(polynomial_pred[polynomial_pred==TRUE])/length(polynomial_pred)*100
  
  radial_pred = y_test == radial_kernel_prediction
  radial_accuracy = length(radial_pred[radial_pred==TRUE])/length(radial_pred)*100
  
  sigmoid_pred = y_test == sigmoid_kernel_prediction
  sigmoid_accuracy = length(sigmoid_pred[sigmoid_pred==TRUE])/length(sigmoid_pred)*100
  
  accuracy_list = c(linear_accuracy, radial_accuracy, polynomial_accuracy, sigmoid_accuracy)
  max_accuracy = max(accuracy_list)
  best_method = ""
  
  if(max_accuracy == linear_accuracy) best_method = "svm-linear"
  if(max_accuracy == radial_accuracy) best_method = "svm-radial"
  if(max_accuracy == polynomial_accuracy) best_method = "svm-poly"
  if(max_accuracy == sigmoid_accuracy) best_method = "svm-sigmoid"
  
  return(list(best_method, max_accuracy, accuracy_list))
}
